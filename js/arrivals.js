...
function getData() {
    ...
    // Create a new XMLHttpRequest to retrieve data from OpenSky Network. 
    let xhrAirport = new XMLHttpRequest();
    xhrAirport.open('GET', `https://cshanor:od1NraVen@opensky-network.org/api/flights/arrival?airport=${airportCode}&begin=${bDate}&end=${eDate}`, true);
    ...
    xhrAirport.onload = function() {
        if (this.status == 200) {
            ...
            for (let i in airportRequestData) {
                ...
                // The 0 there is the key, which sets the date to the epoch. The date is then
                // converted to readable format for display on the web page.
                ...
                outputTitle = '<h2>Arrivals at ' + arrivalAirport + '</h2><br/><br/>';
